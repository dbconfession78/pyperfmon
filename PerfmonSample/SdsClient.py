...
class SdsClient(object):
    ...
    def getType(self, namespace_id, typeId):
        ...
        type = SdsType.from_json(json.loads(response.content))
        response.close()
        return type

    def getTypeReferenceCount(self, namespace_id, typeId):
    ...
    def get_or_create_type(self, namespace_id, type):
        ...
        if response.status_code < 200 or response.status_code >= 300:
            response.close()
            raise SdsError(
                "Failed to create type, {type_id}. {status}:{reason}".format(type_id=type.Id,
    ...
    def deleteType(self, namespace_id, typeId):
    ...
    def getView(self, namespace_id, view_id):
        ...
        view = SdsView.from_json(json.loads(response.content))
        response.close()
        return view
    ...
    def getViewMap(self, namespace_id, view_id):
        ...
        viewMap = SdsViewMap.from_json(json.loads(response.content))
        response.close()
        return viewMap
    ...
    def getViews(self, namespace_id, skip=0, count=100):
        ...
        results = []
        for item in content:
            results.append(SdsView.from_json(item))
        response.close()
        return results
    ...
    def getRangeValues(self, namespace_id, stream_id, value_class, start, skip, count, reverse, boundary_type,
                       view_id=""):
        ...
        values = []
        for c in content:
            values.append(value_class.from_json(c))
        return values
    ...
    def updateValue(self, namespace_id, stream_id, value):
        ...
        if response.status_code < 200 or response.status_code >= 300:
            response.close()
            raise SdsError(
                "Failed to update value for SdsStream, {stream_id}. {status}:{reason}".format(stream_id=stream_id,
                                                                                              status=response.status_code,
                                                                                              reason=response.text))
    ...
    def updateValues(self, namespace_id, stream_id, values):
        ...
        if response.status_code < 200 or response.status_code >= 300:
            response.close()
            raise SdsError(
                "Failed to update all values for SdsStream, {stream_id}. {status}:{reason}".format(stream_id=stream_id,
                                                                                                   status=response.status_code,
                                                                                                   reason=response.text))
    ...
    def removeWindowValues(self, namespace_id, stream_id, start, end):
        ...
        if response.status_code < 200 or response.status_code >= 300:
            response.close()
            raise SdsError(
                "Failed to remove all values for SdsStream, {stream_id}. {status}:{reason}".format(stream_id=stream_id,
                                                                                                   status=response.status_code,
                                                                                                   reason=response.text))
